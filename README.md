# 💻 Kishoor K — Project Portfolio

> 🚀 Passionate Full Stack Developer crafting modern, scalable, and high-performance web applications with clean code, secure backend architecture, and intuitive user experiences.
---

# ✅ Featured Projects

## 🌍 Eterra — Digital Memory Book *(Full Stack MERN)*

> A secure digital platform to preserve life's special moments through photos, videos, and voice memories.

### ✨ Highlights

- 📸 Store photos, videos & voice notes
- 🔐 JWT Authentication & Secure Login
- ☁️ Cloudinary Media Uploads
- 📅 Timeline-based Memory Organization
- 👨‍👩‍👧 Private Family Sharing
- 📱 Fully Responsive MERN Application

**Tech Stack**

`React.js` `Node.js` `Express.js` `MongoDB` `JWT` `Cloudinary`

🔗 **Live Demo:** *(https://eterra-blue.vercel.app/)*

🔗 **Repository:** *(https://github.com/Kishoorkannan/Eterra)*

---

## ✈️ Travyo AI — Intelligent Travel Planner

> A modern AI-powered travel planning platform that helps users discover destinations and plan smarter trips.

### ✨ Highlights

- 🤖 AI Travel Assistant
- 🌦 Live Weather Integration
- 📍 Personalized Travel Planning
- ⚡ REST API Integration
- 📱 Responsive Design

**Tech Stack**

`React.js` `JavaScript` `Weather API` `REST API`

🔗 **Live Demo:** https://travyoai.netlify.app/

🔗 **Repository:** *(https://github.com/Kishoorkannan/Travyo)*

---

## 👶 AI-Powered Baby Monitoring System *(Patent)*

> An intelligent infant monitoring system using Computer Vision and IoT for real-time safety monitoring.

### ✨ Highlights

- 👁 YOLOv8 Object Detection
- 🎤 Cry Detection
- 🌡 Environmental Monitoring
- 🚨 Smart Alert System
- 🤖 AI-Based Risk Detection

**Tech Stack**

`Python` `YOLOv8` `OpenCV` `Arduino` `IoT`

---

# 🛠 Tech Stack

### Frontend

![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Express](https://img.shields.io/badge/Express-black?style=for-the-badge&logo=express)

### Database

![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb)

### Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python)
![Java](https://img.shields.io/badge/Java-007396?style=for-the-badge)

### Tools

Git • GitHub • Antigravity • OpenAI • Postman • Cloudinary

---

# 📫 Connect With Me

📧 Email: kishoorkannan157@gmail.com

💼 LinkedIn: https://www.linkedin.com/in/kishoor-kannan-40399b364/

🌐 Portfolio: https://yourportfolio.com

⭐ Thanks for visiting my profile!
